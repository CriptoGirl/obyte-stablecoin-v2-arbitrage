...
const $get_turnover = (reserve_payout, tokens1, tokens2, p2, reserve_asset_decimals, decimals2) => {
  ...
  if ((tokens1 >= 0 && tokens2 >= 0) || (tokens1 <= 0 && tokens2 <= 0)) return reserve_turnover;
  const token2_turnover = Math.abs(tokens2) * p2 * 10 ** (reserve_asset_decimals - decimals2);
  if ( (tokens2 >= 0 && reserve_payout >= 0) || (tokens2 <= 0 && reserve_payout <= 0) ) {
    return token2_turnover + reserve_turnover;
  }
};

const $get_p2 = (s1, s2, dilution_factor, m, n, isV2) => {
  if (isV2) return Decimal(s1).pow(m).mul(n).mul(Decimal(s2).pow(n - 1)).toNumber()
  else return (dilution_factor * s1 ** m * n *
      (Number.isInteger(n * 2) ? Math.sqrt(s2 ** ((n - 1) * 2)) : s2 ** (n - 1)) );
};

const $get_fee = ( avg_reserve, old_distance, new_distance, fee_multiplier ) => 
    Decimal(fee_multiplier).mul(avg_reserve).mul(new_distance - old_distance).mul(new_distance + old_distance).toNumber()

const $get_reserve = (s1, s2, m, n, dilution_factor, isV2) => {
  if (isV2) return Decimal(s1).pow(m).mul(Decimal(s2).pow(n)).toNumber();
  return Decimal(dilution_factor).mul(Decimal(s1).pow(m).mul(Decimal(s2).pow(n))).toNumber();
};

const $get_growth_factor = ( interest_rate, timestamp, rate_update_ts, growth_factor ) => {
...
const $get_target_p2 = (oracle_price, leverage, interest_rate, timestamp, rate_update_ts, growth_factor ) => {
    if (!oracle_price) return false
    return Decimal(oracle_price).pow(leverage - 1).mul(
        $get_growth_factor(interest_rate, timestamp, rate_update_ts, growth_factor )).toNumber();
};

const $get_distance = (p2, target_p2, isV2) => (p2 && target_p2) ? Math.abs(p2 - target_p2) / (isV2 ? Math.min(p2, target_p2) : target_p2): 0;

const $get_exchange_result = ({
  tokens1, tokens2, tokens_stable,  
  vars, params, oracle_price, timestamp, 
  reservePrice, isV2, addReserve  
}) => {
    let {
        growth_factor,
        reserve = 0,
    ...
    if (distance === 0 && new_distance === 0) {
        ...
        if (regular_fee >= reverse_reward) fee = regular_fee;
        else {
            fee = Decimal.ceil(Decimal(distance_share)
                .div(1 - distance_share * fast_capacity_share)
                .mul(fast_capacity)).toNumber();
        }
    ...
    const s1init = initial_p2
        ? (initial_p2 / (dilution_factor * n *
          (Number.isInteger(n * 2) ? Math.sqrt(s2 ** ((n - 1) * 2)) : s2 ** (n - 1)))) ** (1 / m)
        : (target_p2 / (dilution_factor * n *
          (Number.isInteger(n * 2) ? Math.sqrt(s2 ** ((n - 1) * 2)) : s2 ** (n - 1)))) ** (1 / m);
    ...
    const getReserveNeeded = (T2) => {
        ...
        if (distance === 0 && new_distance === 0) {
            ...
            reverse_reward = Decimal(distance_share).mul(new_fast_capacity).toNumber();
            if (regular_fee >= reverse_reward) fee = regular_fee;
            else fee = Decimal.ceil( Decimal(distance_share)
                    .div(1 - distance_share * fast_capacity_share).mul(fast_capacity)).toNumber();
            reserve_needed = reserve_delta + fee; // negative for payouts
        } else {
            // going towards the target price - get a reward
            fee = 0;
...
exports.$get_exchange_result = $get_exchange_result
